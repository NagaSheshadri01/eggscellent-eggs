...
// 1. Insert newTables exactly after Tables: {
content = content.replace('Tables: {', 'Tables: {' + newTables);

// 2. We find the start and end of `subscriptions: {` manually using brace counting
const subStartIdx = content.indexOf('      subscriptions: {');

let braces = 0;
let started = false;
let subEndIdx = -1;
for (let i = subStartIdx; i < content.length; i++) {
  if (content[i] === '{') {
    braces++;
    started = true;
  } else if (content[i] === '}') {
...
content = content.substring(0, subStartIdx) + '      ' + subsReplacement + content.substring(subEndIdx + 1);
