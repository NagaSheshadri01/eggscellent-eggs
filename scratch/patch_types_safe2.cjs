...
// Ensure every table has Relationships array
// Find all tables in Tables: {
const tablesStart = content.indexOf('Tables: {') + 'Tables: {'.length;
let tablesEnd = -1;
let tBracesCount = 1;
for (let i = tablesStart; i < content.length; i++) {
  if (content[i] === '{') tBracesCount++;
  else if (content[i] === '}') {
    tBracesCount--;
    if (tBracesCount === 0) {
      tablesEnd = i;
...
let tablesStr = content.substring(tablesStart, tablesEnd);
// Replace `        }\n      }` with `        }\n        Relationships: []\n      }` if Relationships is missing
// The structure is:
//       tablename: {
//         Row: { ... }
//         Insert: { ... }
//         Update: { ... }
//       }
// We can use a regex to find missing Relationships
tablesStr = tablesStr.replace(/        Update: \{[\s\S]*?\n        \}\n      \}/g, (match) => {
  if (match.includes('Relationships:')) return match;
  return match.replace(/\n      \}$/, '\n        Relationships: []\n      }');
});

content = content.substring(0, tablesStart) + tablesStr + content.substring(tablesEnd);

fs.writeFileSync(typesPath, content, 'utf8');
console.log('Successfully patched types.ts safely with all Relationships arrays');
